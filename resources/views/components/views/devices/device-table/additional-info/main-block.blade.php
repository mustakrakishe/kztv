    ...
    <div class="container">
        <div name="table-title-row" class="row p-2 justify-content-end">
            ...
            <div name="control-buttons" class="col-4 text-right">
                <x-button value="{{ $device_id }}" onclick="show_new_movement_log_form($(this))">
            ...
            <x-slot name="body">
                @isset($movementHistory)
                    @foreach($movementHistory as $log)
                        <x-views.devices.device-table.additional-info.movement-history-table.rows.log :log="$log" :deviceId="$device_id"/>
                    @endforeach
                @endisset
            </x-slot>
        </x-table>
